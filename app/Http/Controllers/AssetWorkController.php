...
class AssetWorkController extends Controller
{
    ...
    {
        $active = WorkLog::with(['team', 'author'])->where('asset_id', $asset->id)->where('created_by', $request->user()->id)->where('status', 'in_progress')->latest('started_at')->first();

        return view('operator.asset', ['asset' => $asset->load('location'), 'active' => $active, 'types' => WorkLog::TYPES]);
    }

    public function start(Request $request, Asset $asset): RedirectResponse
    {
        $data = $request->validate(['work_type' => ['required', Rule::in(array_keys(WorkLog::TYPES))], 'title' => ['nullable', 'string', 'max:255'], 'description' => ['nullable', 'string', 'max:5000']]);
        ...
        WorkLog::create(['number' => 'BIT-'.now()->format('Ymd').'-'.Str::upper(Str::random(6)), 'created_by' => $request->user()->id, 'work_team_id' => $team->id, 'location_id' => $asset->location_id, 'asset_id' => $asset->id, 'title' => $data['title'] ?? null, 'work_type' => $data['work_type'], 'priority' => 'normal', 'status' => 'in_progress', 'description' => $data['description'] ?? null, 'started_at' => now()]);
